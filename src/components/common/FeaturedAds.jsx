...
export default function FeaturedAds({ isAdmin }) {
  ...
  const handleSave = async (e) => {
    ...
    if (editingId) {
      await updateDoc(doc(db, 'featured_ads', editingId), form);
    } else {
      await addDoc(collection(db, 'featured_ads'), form);
    }
  ...
  const handleImg = async (e) => {
    ...
    if (file) setForm({ ...form, image: await resizeImage(file) });
  };
  ...
  return (
    <section className="px-4 py-16 max-w-[1400px] mx-auto animate-fade-in-up">
      <div className="flex justify-between items-end mb-12 border-b-2 border-brand-primary/10 pb-6">
        ...
        {isAdmin && (
          <button 
            onClick={() => { setForm({ title: '', name: '', whatsapp: '', description: '', image: '', location: '' }); setEditingId(null); setShowForm(true); }}
        ...
        {ads.map(ad => (
          <div key={ad.id} className="relative bg-brand-card rounded-[48px] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-brand-primary/5 flex flex-col md:flex-row group hover:shadow-2xl transition-all duration-700">
            ...
            <div className="md:w-1/2 h-[300px] md:h-auto relative overflow-hidden">
              {ad.image ? (
                <img src={ad.image} alt={ad.title} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
              ) : (
                <div className="w-full h-full bg-brand-primary/5 flex items-center justify-center text-6xl opacity-30">🚀</div>
              )}
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-transparent to-brand-card/20 md:to-brand-card" />
          ...
          <div className="bg-brand-card w-[95%] max-w-[500px] p-5 sm:p-12 rounded-[40px] sm:rounded-[56px] shadow-2xl border border-white/10 relative overflow-hidden animate-pop-in max-h-[95vh] overflow-y-auto no-scrollbar">
            ...
            <div className="text-center">
              ...
              <div className="bg-brand-primary p-5 sm:p-8 rounded-[32px] text-white shadow-xl shadow-brand-primary/20">
                ...
                <p className="font-serif text-3xl sm:text-5xl font-black mb-4 sm:mb-6 tracking-tight">$40.000</p>
                <a 
      ...
      {showForm && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 sm:p-6 bg-brand-dark/60 backdrop-blur-md animate-fade-in">
          <div className="bg-brand-card w-full max-w-[600px] p-8 sm:p-12 rounded-[40px] shadow-2xl border border-brand-primary/10 max-h-[90vh] overflow-y-auto no-scrollbar">
            ...
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-brand-primary ml-2">Imagen</label>
                <div className="relative group">
                  <input type="file" accept="image/*" onChange={handleImg} className="absolute inset-0 opacity-0 cursor-pointer z-10" />
                  <div className="w-full h-40 rounded-3xl border-2 border-dashed border-brand-primary/20 bg-brand-bg flex flex-col items-center justify-center gap-3 transition-all">
                    {form.image ? (
                      <img src={form.image} className="w-full h-full object-cover rounded-3xl" alt="Preview" />
                    ) : (
                      <>
                        <span className="text-4xl">📸</span>
                        <p className="text-[10px] font-black uppercase tracking-widest text-brand-muted">Subir imagen</p>
                      </>
                    )}
                  </div>
                </div>
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button type="submit" disabled={loading} className="flex-2 p-5 rounded-[24px] bg-brand-primary text-white font-black shadow-xl uppercase text-xs tracking-widest">
