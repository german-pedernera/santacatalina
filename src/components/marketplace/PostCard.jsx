...
export default function PostCard({ post, isAdmin, onDelete, onEdit, onApprove, onLike, onClick }) {
  ...
  return (
    <div 
      onClick={onClick}
      className={`bg-brand-card w-full rounded-[24px] shadow-sm hover:shadow-mui overflow-hidden transition-all duration-300 flex flex-col hover:-translate-y-1.5 group animate-fade-in-up cursor-pointer ${!post.approved && isAdmin ? 'ring-2 ring-brand-primary/20' : ''}`}
    >
      {post.photo ? (
      ...
      <div className="p-3 sm:p-4 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-1">
          <h3 className="font-serif text-[0.85rem] sm:text-[1rem] font-black text-brand-dark leading-tight line-clamp-1 flex-1">{post.title}</h3>
          <p className="font-black text-brand-primary text-[0.85rem] sm:text-[1rem] ml-2 shrink-0">
            {Number(post.price) === 0 ? 'Consultar' : `$${Number(post.price).toLocaleString('es-AR')}`}
          </p>
        </div>

        <p className="text-brand-muted text-[0.7rem] sm:text-[0.8rem] mb-3 leading-relaxed flex-1 line-clamp-2">{post.description}</p>
        ...
        <div className="flex flex-col gap-2 mt-auto" onClick={e => e.stopPropagation()}>
          ...
          {isAdmin && (
            <div className="flex gap-2 pt-2 border-t border-brand-primary/10 mt-1">
              {!post.approved && (
                <button className="flex-1 h-9 sm:h-10 flex items-center justify-center rounded-[12px] sm:rounded-[14px] bg-green-500 text-white hover:bg-green-600 btn-mui shadow-lg text-[0.8rem]" onClick={(e) => { e.stopPropagation(); onApprove(post); }} title="Aprobar">✅ Aprobar</button>
              )}
              <button className="flex-1 h-9 sm:h-10 flex items-center justify-center rounded-[12px] sm:rounded-[14px] bg-brand-bg text-brand-dark hover:bg-brand-accent/50 btn-mui text-[0.8rem]" onClick={(e) => { e.stopPropagation(); onEdit(post); }}>✏️ Editar</button>
              <button className="flex-1 h-9 sm:h-10 flex items-center justify-center rounded-[12px] sm:rounded-[14px] bg-[#FEF2F2] text-[#EF4444] hover:bg-[#FEE2E2] btn-mui text-[0.8rem]" onClick={(e) => { e.stopPropagation(); onDelete(post); }}>🗑️ Borrar</button>
            </div>
          )}
        </div>
      </div>
